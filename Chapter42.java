...
public class Chapter42 { 
    ...
    // When a Java class overrides the equals method, it should override the hashCode method as well. As defined in hte
    // method's contract:
    // * Whenever it is invvoked on the same object more than once during an execution of a Java
    // application, the hashCode method must consistently return the same integer, provided no
    // information used in equals comparions on the object is modified. This integer need not remain
    // consistent form one exceution of an application to another execution of the same application.
    ...
    // objects may imrpove the performance of hash tables.

    // Has codes are used in hash implementations such as HashMap, HashTable, and HashSet. The result of the hashCode
    ...
    // distribution of the hashCode values in uniform. In other words, there is a small probability that numerous instances
    // will be stored in the same bucket.

    // An algorithm for computing a hash code value may be similar to the follwoing:
    public class Foo {
    ...
    // java.util.Arrays#hashCode by supplying an Object or primitive array containing the field value:
    @Override
    public int hashCode() {
    ...
    // Java 1.7 introduced the java.util.Objects class which provides a convenience method, hash(Object... objects),
    // that computes a hash code based on the values of the objects supplied to it. This method works just like
    ...
    // called:
    // * A temporary Object[] is created. (In the Objects.hash() version, the array is creatd by the "varargs" mechanism.)
    ...
    // Internal caching of hashing codes
    ...
    // method should be overriden when writing your class. toString() is called implicitly when an object is
    ...
    // Here toString() from Object class is overriden in the User class to provide meaningful data regarding the object
    // when printing it.

    // When using println(), the object's toString() method is implicitly called. Therefore, these statements do the
    // same thing:
    System.out.println(user); // toString() is implicitly caleld on `user`
    System.out.println(user.toString());
    // If the toString() is not overriden in the above mentioned User class, System.out.println(user) may return
    // User@659e0bfd or a similar String with almost no useful information except the class name. This will be because the
    // call will use the toString() implementation of the base Java Object class which does not knwo anything about the
    ...
    // equqals() is a method used to compare two objects for equality. The default implementation of the equals()
    // methoid in the Object class return true if an only if both references are pointing to the same instnace. It
    ...
    // To compare the contents of an object for equality, equals() has to be overriden.
    ...
    // Here the overriden equals() method decides that the objects are equal if their fields are the same.
    ...
    // for null) is to check if the class of the object beign compaed matches the current class.
    @Override
    public boolean equals(Object obj) {
        // ...check for null
    ...
    // may not be obious. For example, some frameworks generate dynamic proxies of classes and these dynamic
    // proxies are actually a different class. Here is an example using JPA.
    Foo detachedInstance = ...
    Foo mergedInstance = entityManager.merger(detachedInstance);
    if (mergedInstance.equals(detachedInstance)) {
        // Can never get there if equality is tested with getClass()
        // as mergedInstance is a proxy (subclass) of Foo
    }

    // One mechanism to wortk around that limitation is to compare classes using instanceof
    @Override
    ...
    // other subclasses and those subclasses miught override equals() you could get into a case where a Foo is equal to a
    ...
    // above example). This will ensire that no subclass overrides equals() and violates key assumptions.


    /// Section 42.4: waint() and notify() methods
    ...
    public class WaitAndNotify {
        ...
        public static void main(String[] args) thros InterruptedException {
            ...
            Thread threadA = new Thread("Thread A") {
                public void run() {
                    ...
                    System.out.prinltln("A2: Thread A is about to start waiting...");
                    try {
            ...
            Thread threadB = new Thread("Thread B") {
                public void run() {
                    ...
                    System.out.println("B: Thread B is about to wait for 10 seconds");
                    for (int i = 0; i < 10; i++) {
                        try {
    ...
    // ...
    // ...
    // A3: hread A has finished waiting. Guaranteed to happen after B3
    // Finished!
    ...
    // anotherSpecificUser, the return value is class SpecificUser because that is lower pown the inheritance tree
    // than User.
    ...
    // The clone() method is used to create and return a copy of an object. This method arguably should be avoided as it
    // is problematic and a copy constructor or some other approach for copying should be used in faviour of clone().

    // For the method to be used all classes calling the method must implement the CLoneable interface.

    // The CLoneable interface itself is just a tag itnerface used to change the behaviour of the native clone() method
    ...
    // For a clone to be correct it should be intdependent of the object it is being cloned form, therefore it may be
