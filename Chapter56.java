...
// the console. The method receives a String with the format information and a series of objects to format:
String rowsStrings[] = new String[] {
        "1",
        "1234",
        "1234567",
        "123456789"};
...
// * %-5s: format the string with a minimum of 5 charaters; if the string is shorter it will be padded to 5
...
// shorter than 5 it will be added to 5 characters and right aligned; if the string is longer than 10 it will be
// truncated to 10 characters and right aligned
// * %-5.5: fornat the string with a fixed size of 5 characters (minimum and maximum are equals); if the string
// is shorter than 5 it will be padded to 5 characters and left aligned; if teh string is longer than 5 it will be
// truncated to 5 characters and left aligned


//// Section 56.3: Implementing Basic Command-Line Behavior

// For basic prototypes of basic command-line behavior, the following loop comes in hando.
public class ExampleCli {

    private static final String CLI_LINE = "example-cli>"; // console linke string
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            e.printStackTrace(); // prints the exception log if it is failed to do get the user input
    ...
    private void start() throws IOException {
        ...
        while (!cmd.equals(CMD_QUIT)) { // terminates console if user input is "quit
            ...
            cmd = reder.readLine(); // takes input from user. user input should be started with
            // "hello", "answer" or "quit"
            String[] cmdArr = cmd.split(" ");

            if (cmdArr[0].equals(CMD_HELLO)) { // exceutes when user input starts with "hello"
                hello(cmdArr);
