public class Chapter130 {
    ...
    // on such resources. Once could argue tha the FileInputStream has a finalizer that ivokes the close() method on a
    // garbage collection event; however, since we can't be sure when a garbage collection cycle will starts, the input
    // stream can consume computer resources for an indefinite period of time. The resource mustb e closed in a
    ...
    // Version >= Java SE 7
    private static void printFileJava7() throws IOEsxception {
    ...
    // itnerface. It ensures that each resource is closed by the end of the statement. The difference between the two
    // interfaces is, that the close() method of Closeable throws an IOException which has to be handled in some way.

    // In cases where the resource has already been opene but should be safely closed after use, one can assign it to a
    // local variable inside the try-with-resources

    // Version > Java SE 7
    private static void printFileJava7(InputStream extResources) throws IOException {
        try (InputStream input = extResource) {
            ... // access resource
        }
    }

    // The local resouce variable created in the try-with-resources constructor is effectively final.


    /// Sectin 130.4: Pitfall: testing a file before attempting to open it

    // Some people recommend that you should apply various tests to a file before attempting to open it either to provide
    // better diagnostics or avoid dealing with exceptions. For example, this method attempts to check if path
    // corresponds to a readable file:
    public static File getVaidatedFile(String path) throws IOException {
    ...
    // The first problem is in the signature for FileInputStream(File) because the compiler will still isnist we catch
    // IOException here, or further up the stack.

    // THe second problem is that checks performed by getValidatedFile do not guarantee that hte FileInputStream
    // will succeed.
    ...
    // message.
    // * There are edge cases not covered by those tests. For example, on a system with SELinux in "eforcing" mode,
    // an attempt to read a file can fail despite canRead() returning true.

    // The third problem is taht the tests are inefficient. For example, the exists, isFile and canRead calls will each make
    ...
    // watned to produice better diagnostics for open failures, you could perform the exists, isFile and canRead checks
    // in the handler.
    ...
    String bat[] = new String[100]; // No member is an object.

    // If you mistakenly think of variables as objects, the actual behavior of the Java langauge will surprise you.
    ...
    // * For Java varriables which have a reference type (either a class or an array type) the variable holds a reference.
    // All copies of a reference are indistinguishable. References may point to objects, or tehy may be null which
    ...
    public final class MutaleLocation {
        public int x;
        public int y;
    ...
    // An instnace of this class is an object that has two fields x and y which ahve the type int.

    // We can have many instnaces of the MutableLocation class. Some will represent the same location in 2D space; i.e.
    ...
    MutableLocaiton elsewhere = new MutaleLocation(1, 2);
    ...
    // If you (incorrectly) think of these variables as being objects, then you are likely to misread the statement as saying:
    ...
    // objects create by the above. The variables here and there actually refer to the same object.

    // We can demonstrate this. Assuming the variable declaratiosn as above:
    System.out.println("BEFOR: here.x is " + here.x + ", there.x is " + there.x +
    ...
    // same objects. But the value that we see via elsewhere.x has not changed, so elsewhere must refer to a different
    // object.
    ...
    // whether two (different) objects are "equaal" in the intuitive sense.
    MutableLocation here = new MutableLocation(1, 2);
    MutableLocation there = here;
    MutalbeLocation elsewhere = new MutableLocation(1, 2);
    ...
    // means thati t is creatinga  copy of the object reference.

    // As long as both objects references are still pointing to the same object, you can modify that object form either
    // reference, an this is what causes confusion for some.
    ...
    void f() {
        MutaleLocation foo = new MutableLocation(0, 0);
        f(foo);
    ...
    // Java manages memory automatically. You are not required to free memory manually. An object's memory of the
    // heap may be freed by a garbage collector when the object is no longer reachable by a live thread.

    // However, you can prevent memory from being freed, by allowing objects to be reachable that are no longe
    r// needed. Whether you call this a memory leak or memory packratting, the result is the same -- an unnecessary
    // increase in allocated memory.

    // Memory leaks in Java can happen in various ways, but the most common reason in everlasting object references,
    // because the garbage collector can't removeo bjects from the heap while there are still references to them.

    // Static fields

    // One can create such a reference by defining class with a static field containing some collection of objects, and
    // forgetting to set that static field to null after the collection is no longer neeed. static fields are considered GC
    ...
    // that that classloader has loaded can be collected. Since each class also referes to its static fields, they cannot be
    // collected either.

    // Accumulation leak

    // The accumulation leak example could look like the following:
    final ScheduledExecturoService scheduledExecturoService = Executors.newScheduledThreadPool(1);
    ...
    scheduledExecturoService.scheduleAtFixedRate(() -> {
    ...
    // This example creates two scheudled tasks. The first task takes the last number from a deque called numbers, and, if
    // the number is divisible by 51, it prints the numebr and the deque's size. The second task puts numbers into the
    ...
    // To prevent this while preserving the semantics of this program, we can use a different method for taking numbers
    // from the deque: pollLast. Contrary to the method peekLast, pollLast returns the element and removes it from
    ...
    // to problems like the one in the folloing example:
    public class Shout {
        public static void main(String[] args) {
            for (String s : args) {
                s.toUpperCase();
    ...
    // So in order to use this method effectively, you needto use the object returned by the method call; for example:
    s = s.toUpperCase();
    // In fact, the "strings never change" rule applies to all String methods. If you remember that, then you can avoid a
    // while category of beginner's mistakes.


    /// Section 130.8: Pitfall: combining assignment and side-effects

    // Occasionally we see StackOverflow Java questions (and C or C++ questions) that ask something like this:
    i += a[i++] + b[i--];
    ...
    // understands how expression evaluation really works in the Java programming language. This is arguable legitimate
    // as a "test of knwoledge", but that does not mean that you should even do this in a real program.

    // To illustrate, the following seemingly simple example has appeared a few time on StackOverflow questions (like this
    // one). In some cases, it appears as a genuine mistake in someone's code.
    int a = 1;
    a = a++;
    System.out.println(a); // What does this print.

    // Most programmers (including Java experts) reeading those statements quickly would say that it outputs 2. In fact, it
    // woutputs 1. For a detailed explanation of why, please read this Answer.

    // However, the real takeway from this and similar examples is that any Java statement that both assigns to and side-
    // effects the same variable is going to be at best hard to understand, and at worst downright misleading. You should
    // avoid writing code like this.

    // 1 - moduel potential issues with the Java Memory Model if the variables or objects are visible to other threads.
