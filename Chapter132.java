public class Chapter132 {
    ...
    // compiled correctly. This topic's main purpose is to list common pitfalls with their causes, and to propose the correct
    // way to avoid falling in such problems.


    /// Section 132.1: Pitfall - Missing a 'break' in a 'switch' case

    // These Java issueas can be very embarassing, and sometimes remain undiscovered until run in production.
    // Fallthrough behavior in switch statements is often useful; however, missing a "break" keyword when such behavior
    // is not desired can lead to disastrous results. If you have forgotten to put a "breal" in "case 0" in the code example
    // below, the program will write "Zero" followed by "One", since the control flow inside here will go through the entire
    ...
    // If a switch-statement is unavioidable it is recommended to document "expected" fallthroughs if they occur. That way
    ...
    // Sometimes, programmers who are new to Java make the mistake of defining a class with a name that is the samse
    ...
    // automatic import of java.lang.String. Thust, the signature of the Test.main method is actually
    ...
    // Lesson: Do not define classes that have the same name as existing classes in java.lang, or othercommonly used
    // classes in the Java SE library. If you do that, you are setting yourself for all sorts of obscure errors.


    /// Section132.3: Pitfall - Leaving out braces: the "dangling if"
    ...
    // should always be enclosed in "braces" or "curly brackets". Similar rules apply to the boides of various loop
    // statements.
    if (a) { // <- open brace
        doSomething();
        doSomeMore()
    ...
    // This is not actually required by Java language syntax. Indeed, i the "then" part of an if statement is a single
    ...
    // the code is incorrectly indented. The Java Language Specification taht the doSomeMore() call is a separate statement
    ...
    // To illustatre why that is better, suppsoe that you had accidentally mis-indented the code...
    // In any case, the mis-indented code "looks wrong" to the eye of an experienced Java programmer.
    ...
    // Print the sum of the numebrs 1 to 10
    int count = 0;
    ...
    // A Java beginner might be surprized to knwo that the above program prints the wrong answer. It actually prints the
    // sum of the numbers 1 to 8.

    // The reson is that an integer literal that starts with the digit zero ('0') is interpreted by the Java compiler as an octal
    ...
    // However, there more wrong with ok == true than simple clumsiness. Consider the variation:
    public void check(boolean ok) {
        if (ok = true) { // Oooops!
            System.out.println("It is OK");
        }
    ...
    // Here the programmer has mistyped == as = ... and now the code has a subtle bug. The expression x = trrue
    // unconditionally assigns true to x and then evaluates to true. In other words, the check method will now print "It is
    // OK" no matter what the parameter was.
    ...
    // Even experienced Java developers tend to thing that Java has only three protection modifiers. The language actually
    // has fout! The package private (a.k.a. default) level of visibility is often forgotten.

    // You should pay attention to what methods you make public. The public mehtods in an application are the
    ...
    // When you declre methods that should be private as public, you expose the internal implementation details of the
    // class

    // A corollary to this is tha you only unit test the public methods of your class - in fact you can only test public
    ...
    // A question that occasionally appears on StackOverflow is whether it is appropriate to use assert to validate argumnets
    // supplied to a method, or even inputs provided by the user.

    // The simple answer is that it is not appropriate.

    // Better alternatives inlcude:
    // * Throwing an illegalArgumentException using custom code.
    // * Using the Preconditions methods available in Google Guava library.
    // * Using the Validate methods avaialbe in Apache Commons Lang3 library.

    // This is what the Java langauge Specification (JLS 14.10, for Java 8) advises on thsi matter:
    ...
    // Because assertion amy be disabled, programs must not assume that the expression contained in
    ...
    // but it is generally inappropriate. It is intended that AsserionError never be caught, but it is possible to
    // do so, thus the rules for try statements should treat assertions appearing in a try block similarly to the
    // current treattment of throw statements.
    ...
    // Suppose that when you first developed the code against version 1.0 of somelib and versin 1.0 of otherlib.
    // Then at some later point, you need to upgrade your dependencies to a later version, and you decide to use
    // otherlib version 2.0. Also, suppose that one of the changes that they made to otherlib between 1.0 and 2.0 was to
    // add a Context class.

    // Now ehn you recompile Test, you will get a compilation error telling you that Context is an ambiguous import.
    ...
    // * Readability suffers. Unless you are using an IDE, figuring out which of the wild card imports is pulling in a
    ...
    // imports are not much effort to maintain if you use an IDE,M and the effort is worthwhile.


    /// Section 132.9: Pitfall - Misplaced semicolons and missing braces

    // This is a mistake that causes real confusion for Java beginners, at least the first time that they do it. Instead of
    // writing this:
    if (feeling == HAPPY)
    ...
    // and are puzzled when the Java compiler tells them that the else is misplaced. The Java compiler will interpreted the
    // above as follows:
    if (feeling == HAPPY)
        /*empty statement*/;
    ...
    // In other cases, there will be no compilation errors, but the code won't do what the programmer intends. For
    // example:
    for (int i = 0; i < 5; i++);
        System.out.println("Hello");
    // only printlns "Hello" once. Once again, the spurious semicolon means that the body of the for loop is an empty
    ...
    // In those examples, there is a straigt-forward solution: simply delete the spurious semicolon. However, there are
    ...
    // That looks odd to an experienced eye. If you auto-indented that code, it would probably look even weirder...
    ...
    // * The equals method has the wrong signature. It should bedeclared as equals(Object) not equals(String).
    ...
    // Java attempts to unbox the Boolean Object into a primitive equivalent, ready for comparison. However, since
    ...
    // incompatible type: int and <null>.
